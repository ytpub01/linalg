...
for N=Nvalues
    ...
    [u1 flag relres iter rkperN]=mysiedel(A1,b1,tol,maxiter);
    ...
    title(['Solution to the PDE at iteration k=' num2str(iter)...
        ' for N=' num2str(N)]);
    legend('exact','discretized','Gauss-Siedel','Location','SouthWest');
end
...
figure
plot(1:rkiter(1),log10(rk(1,1:rkiter(1,1))),1:rkiter(2),...
    log10(rk(2,1:rkiter(2))),1:rkiter(3),log10(rk(3,1:rkiter(3))),...
    1:rkiter(4),log10(rk(4,1:rkiter(4))));
xlabel('k');
ylabel('log10 ( norm ( r_k ) )');
title('L2 norm of the Gauss-Siedel residual at each iteration');
